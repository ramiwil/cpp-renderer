  ...
  for (auto &light : sc.lights) {
    ...
    if (!occluded) {
      ...
      if (cos_surface > 0 && cos_light > 0) {
        ...
        Vec3 f =
            bxdf->evaluate(closest.normal, dir_to_light, -ray.get_direction());
        f_direct +=
            f * Le * cos_surface * cos_light / (dist_sq * light->pdf_area());
      }
    }
  }
...
Scene build_cornell_box() {
  ...
  sc.add_object(
      std::make_unique<Sphere>(mat_sphere, Vec3(0.0f, -radius, 0.0f), radius));
