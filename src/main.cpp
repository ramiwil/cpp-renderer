...
Scene build_cornell_box() {
	...
	sc.add_object(std::make_unique<Sphere>(mat_sphere, Vec3(0.0f, -radius, 0.0f), radius));

	// create light
	auto mat_light = std::make_shared<LambertianMaterial>(Vec3(1.0f));
	mat_light->emission = Vec3(1.0f);
	mat_light->emission_strength = 10.0f;
	sc.add_light(std::make_unique<Light>(mat_light, Vec3(0.0, 49.9, 0.0), Vec3(0.0, 1.0, 0.0),
										 20.0f, 20.0f));
	...
	auto mat_green = std::make_shared<LambertianMaterial>(Vec3(0.0f, 1.0f, 0.0f));
	...
	sc.add_object(std::make_unique<Plane>(mat_red, Vec3(size / 2.0, 0.0f, 0.0f),
										  ...
										  size)); // red wall
	sc.add_object(std::make_unique<Plane>(mat_green, Vec3(-size / 2.0, 0.0f, 0.0f),
										  Vec3(1.0, 0.0, 0.0), size,
										  size)); // green wall
...
#pragma omp parallel for schedule(dynamic, 16)
	for (int y = 0; y < HEIGHT; y++) {
		for (int x = 0; x < WIDTH; x++) {
			...
			pixel_color /= samples; // average the final folor by the number of samples
	...
	stbi_write_png("renders/output.png", WIDTH, HEIGHT, 3, pixels.data(), WIDTH * 3);
