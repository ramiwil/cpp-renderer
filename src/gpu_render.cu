    ...
    if ((row < sp.height) && (col < sp.width)) {
        ...
        for (int smp = 0; smp < sp.samples; smp++) {
            ...
            for (int d = 0; d < sp.depth; d++) {
                ...
                for (int light = 0; light < sp.num_lights; light++) {
                    ...
                    if (!(occluded_s || occ_b)) {
                        ...
                        if (cos_surface > 0 && cos_light > 0) {
                            ...
                            Vec3 f = evaluate_material(obj_material, closest.normal, dir_to_light, ray.get_negative_direction(), &localState);
                            f_direct += f * Le * cos_surface * cos_light / (dist_sq * (1.0f / (l.l * l.w)));
                        }
                    }
                }
                ...
                pixel_color += throughput * (obj_material.emission * obj_material.emission_strength + f_direct);
